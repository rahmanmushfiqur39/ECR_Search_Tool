...
if repd_df is not None and ecr_df is not None:
    ...
    if start_id and end_id and st.button("🔍 Run Matching"):
        ...
        # --- Prepare ECR ---
        ecr = ecr_df[ecr_df["Energy Source 1"].str.lower() == "solar"].copy()
        ...
        try:
            ...
            ecr_gdf = gpd.GeoDataFrame(
                ecr,
                geometry=gpd.points_from_xy(ecr["Location (X-coordinate): Eastings (where data is held)"],
                                            ecr["Location (y-coordinate): Northings (where data is held)"],
                                            crs="EPSG:27700")
            )
        except Exception as e:
        ...
        for idx, repd_row in repd_gdf.iterrows():
            ...
            for _, ecr_row in ecr_candidates.iterrows():
                ...
                ecr_cap = ecr_row.get("Accepted to Connect Registered Capacity (MW)")
                ...
                # Text Group A
                repd_text_a = str(repd_row.get("REPD Operator (or Applicant)", "")) + " " + str(repd_row.get("Site Name", ""))
                ecr_text_a = str(ecr_row.get("ECR Customer Name", "")) + " " + str(ecr_row.get("Customer Site", ""))
                ...
                # Text Group B
                repd_text_b = str(repd_row.get("REPD Site Name", "")) + " " + str(repd_row.get("Address", ""))
                ecr_text_b = str(ecr_row.get("Customer Site", "")) + " " + str(ecr_row.get("Address Line 1", ""))
        ...
        # Save to Excel
        out_file = "matched_results.xlsx"
        results.to_excel(out_file, index=False)

        with open(out_file, "rb") as f:
            st.download_button("📥 Download Results as Excel", f, file_name="matched_results.xlsx")
